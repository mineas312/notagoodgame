...
template<class T = typename std::enable_if<std::numeric_limits<T>::is_floating_point, bool>::type>
inline bool isEqf(const T a, const T b, const uint8_t ulp) noexcept
{
	return std::abs(a - b) <= std::numeric_limits<T>::epsilon() * std::abs(a + b) * ulp || std::abs(a - b) < std::numeric_limits<T>::min();
}

template<class T = typename std::enable_if<std::numeric_limits<T>::is_arithmetic, bool>::type>
